...
public class LigneProdControllerTest {
    ...
    public void testAddNewLigneProd() throws CodeLigneProdExistException, LigneProdNotFoundException, UniteFabNotFoundException {
        ...
        when(ligneProdService.addLigneProd(eq(codeLp), eq(robotTraitement),eq(operation), eq(observation), anyBoolean(), eq(codeUf)))
                .thenReturn(expected);

        ResponseEntity<LigneProduction> response = ligneProdController.addNewLigneProd(codeLp, robotTraitement,operation, observation, status, codeUf);
    ...
    public void testUpdateLigneProd() throws CodeLigneProdExistException, LigneProdNotFoundException, UniteFabNotFoundException {
        ...
        when(ligneProdService.updateLigneProd(eq(codeLp), eq(robotTraitement),eq(operation), anyBoolean()))
                .thenReturn(expected);

        ResponseEntity<LigneProduction> response = ligneProdController.updateLigneProd(codeLp, robotTraitement,operation, status);

...
export class LigneProdctionService {
  ...
  public getGlobalTRG(): Observable<number> {
    return this.http.get<number>(`${this.host}/produit/global-trg`);
  }

  getTRGByShifts(): Observable<Map<string, number>> {
    return this.http.get<Map<string, number>>(`${this.host}/trg-by-shifts`);
  }
  
  getTRGByLigneProduction(): Observable<Map<string, number>> {
    return this.http.get<Map<string, number>>(`${this.host}/produit/trg-by-ligne`);
  }

  getTRGByProduit(): Observable<Map<string, number>> {
    return this.http.get<Map<string, number>>(`${this.host}/produit/trg-by-produit`);
  }
  ...
  public createFormData(ligneProd : LigneProduction) : FormData {
    ...
    formData.append('operation',ligneProd.operation) ;
